...
class KTTheme:
    ...
    def printHtmlAttributes(scope):
        ...
        if scope in KTTheme.htmlAttributes:
            for key in KTTheme.htmlAttributes[scope]:
                attributes.append(
                    '{}="{}"'.format(key, KTTheme.htmlAttributes[scope][key])
                )
    ...
    def setLayout(view, context={}):
        ...
        # Get module path
        module = "_templates.{}._bootstrap.{}".format(
            settings.KT_THEME_LAYOUT_DIR.replace("/", "."), layout
        )
        ...
        if not util.find_spec(module) == None:
            ...
            KTBootstrap = KTTheme.importClass(
                module, "KTBootstrap{}".format(layout.title().replace("-", ""))
            )
            KTBootstrap.init(context)
        else:
            module = "_templates.{}._bootstrap.default".format(
                settings.KT_THEME_LAYOUT_DIR.replace("/", ".")
            )
    ...
    def getIcon(name, class_name="", type=""):
        ...
        if not type:
            type = getattr(settings, "KT_THEME_ICONS", "duotone")

        if type == "duotone":
            icons = cache.get("duotone-icons")
            if not icons:
                with open(settings.BASE_DIR / "_keenthemes/libs/icons.json") as f:
                    icons = json.load(f)
                cache.set("duotone-icons", icons, 3600)

            paths_number = icons.get("duotone-paths", {}).get(name, 0)

            output = f'<{tag} class="ki-{type} ki-{name}{" " + class_name if class_name else ""}">'
            for i in range(paths_number):
                output += f'<{tag} class="path{i+1}"></{tag}>'
            output += f"</{tag}>"
        else:
